...
# Patroon #1 uit Commandocentrums Patronen-overzicht (oorsprong: Spil).
# Eigen JSON-bestand (config.VRAGEN_PAD), geen "wie"-veld -- dit dashboard
# heeft geen gebruikersaccounts.

_VRAGEN_STATUSSEN = ("open", "beantwoord", "bouwen", "gebouwd", "toegelicht", "afgewezen")


def _vragen_lezen() -> list[dict]:
    if not os.path.exists(config.VRAGEN_PAD):
        return []
    with open(config.VRAGEN_PAD, "r", encoding="utf-8") as f:
        return json.load(f)


def _vragen_schrijven(vragen: list[dict]) -> None:
    os.makedirs(os.path.dirname(config.VRAGEN_PAD), exist_ok=True)
    with open(config.VRAGEN_PAD, "w", encoding="utf-8") as f:
        json.dump(vragen, f, ensure_ascii=False, indent=2)


def alle_vragen(status: str = "", gebied: str = "") -> list[dict]:
    vragen = _vragen_lezen()
    if status:
        vragen = [v for v in vragen if v["status"] == status]
    if gebied:
        vragen = [v for v in vragen if (v.get("gebied") or "") == gebied]
    return sorted(vragen, key=lambda v: v["id"], reverse=True)


def alle_vragen_gebieden() -> list[str]:
    return sorted({v["gebied"] for v in _vragen_lezen() if v.get("gebied")})


def vragen_statistiek() -> dict:
    telling = {s: 0 for s in _VRAGEN_STATUSSEN}
    for v in _vragen_lezen():
        telling[v["status"]] = telling.get(v["status"], 0) + 1
    return telling


def vraag_toevoegen(onderwerp: str, vraag: str, gebied: str = "", toelichting: str = "", prioriteit: str = "normaal") -> dict:
    vragen = _vragen_lezen()
    nieuw_id = max((v["id"] for v in vragen), default=0) + 1
    item = {
        "id": nieuw_id, "onderwerp": onderwerp, "gebied": gebied, "vraag": vraag,
        "toelichting": toelichting, "prioriteit": prioriteit,
        "antwoord": None, "actie": None, "status": "open",
        "aangemaakt_op": datetime.now().isoformat(), "beantwoord_op": None,
        "klaar_om_te_bouwen_op": None, "verwerkt_op": None, "afgewezen_op": None,
        "verwerkt_in_versie": None,
    }
    vragen.append(item)
    _vragen_schrijven(vragen)
    return item


def _vraag_zet(vraag_id: int, velden: dict) -> dict | None:
    vragen = _vragen_lezen()
    for v in vragen:
        if v["id"] == vraag_id:
            v.update(velden)
...
def vraag_beantwoorden(vraag_id: int, antwoord: str, actie: str = "") -> dict | None:
    return _vraag_zet(vraag_id, {
        "antwoord": antwoord, "actie": actie, "status": "beantwoord",
        "beantwoord_op": datetime.now().isoformat(),
    })


def vraag_klaar_om_te_bouwen(vraag_id: int) -> dict | None:
    return _vraag_zet(vraag_id, {"status": "bouwen", "klaar_om_te_bouwen_op": datetime.now().isoformat()})


def vraag_gebouwd(vraag_id: int, versie: str, actie: str = "") -> dict | None:
    velden = {"status": "gebouwd", "verwerkt_op": datetime.now().isoformat(), "verwerkt_in_versie": versie}
    if actie:
        velden["actie"] = actie
    return _vraag_zet(vraag_id, velden)


def vraag_toegelicht(vraag_id: int, versie: str = "") -> dict | None:
    return _vraag_zet(vraag_id, {
        "status": "toegelicht", "verwerkt_op": datetime.now().isoformat(), "verwerkt_in_versie": versie,
    })


def vraag_afwijzen(vraag_id: int, reden: str = "") -> dict | None:
    velden = {"status": "afgewezen", "afgewezen_op": datetime.now().isoformat()}
    if reden:
        velden["antwoord"] = reden
    return _vraag_zet(vraag_id, velden)
